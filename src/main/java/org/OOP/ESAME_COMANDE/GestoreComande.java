...
class GestoreComande extends Gestore<Ordine>{
    ...
    void rimuovi(Ordine daRimuovere){
        for(Map.Entry<Integer, List<Ordine>> comanda: comandePagate.entrySet()){
        ...
        for (Map.Entry<Integer, List<Ordine>> comanda: comandeAperte.entrySet()){
            if(comanda.getValue().contains(daRimuovere)) {
                comanda.getValue().remove(daRimuovere);
                assert(!comandePagate.values().contains(daRimuovere));
                return;
    ...
    /**
     * (Observer).
     * L'elenco delle pietanze ordinate ma non consegnate suddivise per tavolo
     * @return
     */
